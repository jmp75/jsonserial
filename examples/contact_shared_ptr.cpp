//
//  contact_shared_ptr.cpp: JsonSerial Example with STD SHARED POINTERS
//
...
#include "contact_shared_ptr.hpp"
using namespace std;
...
public:
  ...
  MyClasses() {
    defclass<Phone>("Phone")
    .member("type", &Phone::type)
...
int main(int argc, char** argv)
{
  ...
  std::shared_ptr<Contact::Address> address
  (new Contact::Address{"21 2nd Street","New York", "NY", "10021-3100"});
  
  std::shared_ptr<Phone> phone(new Phone("home","123 456-7890"));
  
  std::shared_ptr<Contact> bob(new Contact("Bob", "Smith", 37, Contact::Male));
  bob->addAddress(address);
  bob->addPhone(phone);
  
  std::shared_ptr<Contact> alice(new Contact("Alice", "Smith", 38, Contact::Female));
  alice->addAddress(address);
  alice->addPhone(phone);
  alice->addPhone(std::make_shared<Phone>("office","703 221-2121"));
  
  std::shared_ptr<Contact> laura(new Contact("Laura", "Smith", 10, Contact::Female));
  laura->addAddress(address);
  laura->addPhone(phone);
  laura->addPhone(std::make_shared<Phone>("mobile","211 1234-2121"));
  
  std::shared_ptr<Contact> franck(new Contact("Franck", "Smith", 6, Contact::Male));
  ...
  std::shared_ptr<PhotoContact> brad(new PhotoContact("Brad", "Smith", 12, Contact::Male));
  ...
  std::shared_ptr<Contact> bobcopy_simple;
  std::shared_ptr<Contact> bobcopy_poly;
  std::shared_ptr<Contact> bobcopy_cyclic;
